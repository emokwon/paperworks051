...
- OS: Windows 11
- 셸: PowerShell (기본), Git Bash 병용 가능
- 작업 디렉토리: 프로젝트 루트 기준으로 실행할 것
- 경로 구분자는 `\`이며, PowerShell에서는 `&&`/`||` 체이닝이 안 되므로 `;` 또는 `if ($?) { ... }` 사용
...
- **프로젝트 이름:** web test
- **기술 스택:** Vanilla HTML / CSS / JS (빌드 도구·프레임워크 없음)
- **개발 서버 실행:** `powershell -NoProfile -ExecutionPolicy Bypass -File .devserver.ps1` (http://localhost:5173, 로컬 미리보기 전용 정적 서버) — 또는 `index.html`을 브라우저에서 바로 열어도 됨
- **빌드:** 없음 (정적 파일 그대로 배포)
- **테스트:** 자동화 테스트 없음. 브라우저에서 품목 추가/삭제·합계 계산·인쇄 미리보기를 수동 확인
- **배포:** `git push origin main` (자동 배포 예정 시)
- **주의사항:** `.devserver.ps1`은 로컬 미리보기용 헬퍼 스크립트로 배포 대상 아님

### 사이트 구조

- `index.html` — 홈(양식 선택) 페이지, `home.css`. "무료 견적서 양식" → `estimate.html`, "무료 간이영수증 양식" → `receipt.html`로 이동하는 카드 2개
- `estimate.html` + `style.css` + `script.js` — 견적서 작성 페이지
  - 공급자/공급받는자 정보, 품목 테이블(수량·단가·자동 금액 계산), 공급가액/부가세(10%)/합계 자동 산출
  - 계약금·잔금 비율(%) 입력 시 금액 자동 계산, 공급자 도장/사인 이미지 업로드(회사명 옆 표시, 인쇄 시에도 유지)
  - 품목 추가(`+ 품목 추가`)·삭제(행별 `✕`) 가능
  - `인쇄 / PDF 저장` 버튼으로 `window.print()` 호출 — 인쇄 시 버튼·삭제열은 숨김 처리(`@media print`)
- `receipt.html` + `style.css` + `receipt.css` + `receipt.js` — 간이영수증 작성 페이지 (사용자 제공 영수증 양식 참고)
  - 공급자 정보(사업자등록번호/상호/성명·도장/사업장 소재지/업태·종목), 작성년월일/공급대가총액/비고, 영수·청구 선택
  - 품목 테이블(월일/품목/수량/단가/금액 자동 계산 및 합계)
- 각 작성 페이지 상단 툴바에 "← 처음으로" 링크로 `index.html`로 돌아갈 수 있음
